#!/usr/bin/env python3
# Déplacé depuis Test/ vers tests/legacy/ pour unifier la suite de tests.
# Ces tests n'interfèrent pas avec l'API/BDD NAS; gardés comme ressources de validation locale.
...
def test_response_comparison():
    test_cases = [
        {"question": "Bonjour, comment allez-vous ?", "response": "Je vais très bien, merci de votre question !"},
        {"question": "Pourquoi le code ne fonctionne pas ?", "response": "Il faut vérifier la syntaxe et les indentations."},
        {"question": "Merci pour votre aide !", "response": "De rien, c'était avec plaisir."},
        {"question": "Peux-tu m'expliquer comment faire une boucle en Python ?", "response": "Utilisez 'for' pour itérer sur une séquence ou 'while' pour une condition."},
        {"question": "J'ai un problème avec mon serveur", "response": "Vérifiez les logs et la configuration réseau."},
    ]
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        for mode in modes:
            ...
            result = system.get_enhanced_response(test_case['response'], test_case['question'], 0.85)
